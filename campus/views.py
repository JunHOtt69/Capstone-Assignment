...
def generate_timetable(request):
    ...
    # Track what's booked this week: sets of (session_id, date)
    ...
    for ex in existing_qs:
        existing_bookings.add((ex.session_id, ex.date, 'intake', ex.term_id))
        existing_bookings.add((ex.session_id, ex.date, 'lecturer', ex.lecturer_id))
        existing_bookings.add((ex.session_id, ex.date, 'facility', ex.session.facility_id))

    # Build a more efficient clash-check structure
    booked_intake_slots = set()
    booked_lecturer_slots = set()
    booked_facility_slots = set()
    for ex in existing_qs.select_related('session'):
        key = (ex.session.day_of_week, ex.session.start_time, ex.date)
...
def rearrange_missing_class(request):
    ...
    facility_type = 'Lab' if subj.class_type == 'Lab' else 'Classroom'
